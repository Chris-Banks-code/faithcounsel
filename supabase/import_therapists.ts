 ...
 * Run: npx tsx supabase/import_therapists.ts
 */
...
async function importTherapists(rows: TherapistRow[]) {
  ...
  for (let i = 0; i < therapists.length; i += chunkSize) {
    ...
    if (error) {
      console.error(`  Batch ${i / chunkSize + 1} error:`, error.message);
      errors += chunk.length;
    } else {
...
async function main() {
  ...
  const { error: deleteError } = await supabase.from("therapists").delete().neq("id", "00000000-0000-0000-0000-000000000000");
  if (deleteError) {
